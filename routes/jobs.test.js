...
//     expect(resp.body.job.title).toEqual("job1");
//     expect(resp.body.job.salary).toEqual(100000);
//     expect(resp.body.job.equity).toEqual(0.5);
...
describe("GET /jobs?", function(){
    ...
    test("works for users: filter jobs shown", async function(){
        const resp = await request(app).get('/jobs?title=job1&minSalary=50000&equity=true');
        expect(resp.body).toEqual({
            results:[
                {
                    title: "job1",
